load('hmm_Generated_OneNode_TraceLength.mat');
load('hmm_OriginTraceLength.mat');

a1 = max(OriginTraceLength);
a2 = max(GeneratedTraceLength_OneNode);
...
h2 = histogram(GeneratedTraceLength_OneNode, edges, 'Normalization', 'probability');
h2.FaceColor = 'yellow';
h2.FaceAlpha = 0.5;
...
print('Histogram_hmm_TraceLength_OneNode','-depsc');

mean(OriginTraceLength)
mean(GeneratedTraceLength_OneNode)
std(OriginTraceLength)
std(GeneratedTraceLength_OneNode)